  ...
  <AuthLayout title="Mot de passe oublié ?" :show-sidebar="false">
    ...
    <template #default>
      ...
      <form @submit.prevent="submitForm" class="">
        <div class="grid gap-6 pb-2" :style="{ maxWidth: '350px' }">
          <Label for="email">Email</Label>
          <Input id="email" v-model="email" type="email" required :disabled="isSubmitting" />
          <div class="text-justify pt-2 pb-6 mx-4">
            <p className="text-sm text-muted-foreground ">
              Entrez votre adresse e-mail pour recevoir les instructions de réinitialisation de mot
              de passe.
            </p>
          </div>
        </div>
    ...
    <template #links>
      <div class="text-center text-sm">
        ...
        <a :href="links.register" class="underline underline-offset-4 hover:text-primary"
          >S'inscrire</a
        >
      </div>
    </template>
