...
import { ref } from 'vue'
import { Edit, LogOut, CalendarDays } from 'lucide-vue-next'
import { NAV_BTN, NAV_ICON, NAV_AVATAR } from '@/components/shared/sidebar/navStyles'
...
<template>
  <SidebarMenu>
    <SidebarMenuItem>
      <!-- Log Out  -->
      <DecoDialog :on-confirm="logout" title="Log Out" description="Are you sure you want to log out?"
        action-label="Continue" cancel-label="Cancel" :destructive="true">
        <template #trigger>
          <SidebarMenuButton as-child>
            <button type="button" :class="['group', NAV_BTN]" @click.stop>
    ...
    <SidebarMenuItem>
      <SidebarMenuButton as-child>
        <button type="button" :class="['group', NAV_BTN, 'cursor-pointer']" @click="openEditProfile">
          <Edit :class="NAV_ICON" />
    ...
    <SidebarMenuItem>
      <HoverCard :open-delay="100" :close-delay="100">
        ...
        <HoverCardContent side="right" align="start" :side-offset="8" class="w-64 p-4 z-50 m-2">
          <div class="flex items-center gap-3">
            ...
            <div class="space-y-1">
              ...
              <Label class="text-muted-foreground text-xs">{{ props.user?.email || 'john@example.com' }}</Label>
            </div>
          </div>

          <div class="bg-border h-px my-3"></div>
