...
<template>
  <div class="w-80 flex flex-col space-y-6 p-6">
    <!-- Actions rapides -->
    <div
      class="bg-white/80 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-lg dark:shadow-xl border border-white/20 dark:border-slate-700/50 p-6 transition-all duration-300 hover:shadow-xl dark:hover:shadow-slate-900/50">
      ...
      <div class="grid grid-cols-2 gap-3">
        <button v-for="action in quickActions" :key="action.id"
          ...
          <Icon :name="action.icon"
            :class="['w-6 h-6 mb-2 group-hover:scale-110 transition-transform duration-300', action.color]" />
    ...
    <div
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center space-x-2">
          <span class="text-xs text-gray-500 dark:text-gray-400">{{todos.filter(t => !t.completed).length}}/{{
            todos.length }}</span>
          <Icon name="plus"
            class="w-5 h-5 text-gray-400 dark:text-gray-500 hover:text-violet-500 dark:hover:text-violet-400 cursor-pointer transition-colors duration-300" />
        </div>
      </div>
      ...
      <div class="space-y-3 max-h-48 overflow-y-auto scrollbar-hide">
        <div v-for="todo in todos" :key="todo.id"
          ...
          <div :class="[
            ...
            <Icon v-if="todo.completed" name="check" class="w-3 h-3 text-white" />
          </div>
    ...
    <div
      ...
      <div class="space-y-4 max-h-64 overflow-y-auto scrollbar-hide">
        <div v-for="notification in notifications" :key="notification.id"
          ...
          <div
            :class="['flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-1', notification.type === 'course' ? 'bg-blue-100 dark:bg-blue-900/30' : notification.type === 'assignment' ? 'bg-orange-100 dark:bg-orange-900/30' : notification.type === 'message' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-yellow-100 dark:bg-yellow-900/30']">
            <Icon :name="notification.icon"
              :class="['w-4 h-4 transition-colors duration-300', getNotificationColor(notification.type)]" />
          </div>
          <div class="flex-1 min-w-0">
            <p
              class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors duration-300">
              {{ notification.title }}
            </p>
          ...
          <div
            class="w-2 h-2 bg-violet-400 dark:bg-violet-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          </div>
        </div>
      </div>
    </div>
