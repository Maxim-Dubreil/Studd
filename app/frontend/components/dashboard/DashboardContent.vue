...
<template>
  <div class="flex-1 p-8">
    ...
    <div class="grid grid-cols-12 gap-6 mb-8">
      ...
      <div class="col-span-12 h-40 bg-gradient-to-r from-violet-500 to-blue-600 dark:from-violet-600 dark:to-blue-700 rounded-3xl shadow-lg dark:shadow-2xl p-8 flex items-center justify-between text-white transition-all duration-300 hover:shadow-xl dark:hover:shadow-violet-500/25 hover:scale-[1.02]">
        <div class="flex-1">
          <h2 class="text-2xl font-bold mb-3">Continuez votre apprentissage !</h2>
          <p class="text-violet-100 dark:text-violet-200 text-lg">Vous êtes à {{ stats.progressPercent }}% de vos objectifs cette semaine</p>
          <div class="mt-4">
      ...
      <div class="col-span-3 bg-white/80 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-lg dark:shadow-xl border border-white/20 dark:border-slate-700/50 p-6 h-36 transition-all duration-300 hover:shadow-xl dark:hover:shadow-slate-900/50 hover:scale-105">
        ...
        <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-3">
          <div class="bg-gradient-to-r from-green-400 to-green-600 dark:from-green-500 dark:to-green-700 h-3 rounded-full transition-all duration-500" :style="`width: ${stats.progressPercent}%`"></div>
        </div>
      </div>
      ...
      <div class="col-span-12 bg-white/80 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-lg dark:shadow-xl border border-white/20 dark:border-slate-700/50 p-8 transition-all duration-300 hover:shadow-xl dark:hover:shadow-slate-900/50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center space-x-3">
                <div :class="['p-2 rounded-lg bg-white/20 dark:bg-black/20', item.type === 'break' ? 'bg-gray-200 dark:bg-gray-700' : '']">
              ...
              <div v-if="item.type !== 'break'" class="w-3 h-3 bg-current rounded-full group-hover:scale-125 transition-transform duration-300"></div>
            </div>
