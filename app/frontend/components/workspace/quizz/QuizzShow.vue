    ...
    <div class="flex align-center justify-center flex-col">
      ...
      <div v-if="!isStartScreen && !isCompleted" class="mx-auto w-full max-w-6xl p-6">
        ...
        <div class="mb-6 text-center">
          ...
          <p class="text-muted-foreground text-sm">Session {{ currentSession }}</p>
        </div>
        ...
        <div class="mx-auto max-w-2xl">
          <div class="mb-2 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <Icon name="clock" class="text-muted-foreground h-4 w-4" />
              <span class="text-foreground text-sm font-medium">{{ timer }}</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-muted-foreground text-sm">
                Question {{ currentQuestionIndex + 1 }} of {{ quiz.questions.length }}
              </span>
              <span class="text-foreground text-sm font-medium">{{ progressPercentage }}%</span>
            </div>
          </div>
      ...
      <div class="py-10 flex items-center justify-center">
        ...
        <div v-else class="] mx-auto flex flex-col">
              ...
              <div v-if="currentQuestion && !isCompleted" class="flex flex-col p-6">
                ...
                <div class="mb-6 flex items-center justify-center gap-3">
                  ...
                  >
                    ...
                    <Icon
                      :name="questionValidated ? 'chevron-right' : 'check'"
                      class="ml-1 h-4 w-4"
                    />
                  </Button>
                  ...
                  >
                    ...
                    <Icon :name="questionValidated ? 'flag' : 'check'" class="ml-1 h-4 w-4" />
                  </Button>
                  ...
                  >
                    Finish
                    <Icon name="flag" class="ml-1 h-4 w-4" />
                  </Button>
                </div>

                <!-- Separator between navigation and answers -->
                <div class="bg-border mb-6 h-px w-full"></div>
                ...
                >
                  <div class="space-y-4 pb-4">
                    <div
                      ...
                      :key="option.id"
                      class="flex cursor-pointer items-start rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-colors hover:bg-gray-50"
                      :class="{
                    ...
                    >
                      <div
                        class="mt-0.5 mr-3 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full border-2"
                        :class="{
                      ...
                      >
                        <div
                          v-if="selectedAnswers.includes(option.id)"
                ...
                >
                  <div class="space-y-4 pb-4">
                    <div
                      ...
                      :key="option.id"
                      class="flex cursor-pointer items-center rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-colors hover:bg-gray-50"
                      :class="{
                        'border-purple-500 bg-purple-50 shadow-md shadow-purple-200':
                          selectedAnswers.includes(option.id),
                        'border-gray-200': !selectedAnswers.includes(option.id),
                ...
                >
                  <h3
                    ...
                    :class="isCurrentAnswerCorrect ? 'text-green-800' : 'text-red-800'"
                  >
              ...
              <div v-if="isCompleted && !testMode" class="py-10 flex flex-col p-6">
                <div class="max-w-xl">
                  ...
                  <div class="mb-8 text-center">
                    <div
                      class="mb-4 inline-flex h-20 w-20 items-center justify-center rounded-full bg-green-100"
                    >
                      <Icon name="check-circle" class="h-10 w-10 text-green-600" />
                    </div>
                    <h2 class="text-foreground mb-2 text-3xl font-bold">Learning Complete!</h2>
                    <p class="text-muted-foreground text-lg">
                      Great job on completing the learning session
                    </p>
                  </div>
                  ...
                  >
                    <div class="flex items-center justify-between">
                      <div>
                        <h3 class="mb-1 text-lg font-semibold text-green-800">Your Performance</h3>
                        <p class="text-sm text-green-600">Questions answered correctly</p>
                      </div>
                    ...
                    <div class="mt-4">
                      ...
                      <div class="h-2 w-full rounded-full bg-green-200">
                        <div
                          ...
                          :style="{
                            width: Math.round((score / quiz.questions.length) * 100) + '%',
                          }"
                        ></div>
                      </div>
                    </div>
          ...
          <div v-if="isCompleted && testMode" class="flex flex-1 flex-col p-6">
            <!-- Header with Score -->
            <div class="mb-6 text-center">
              <div
                class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full"
                :class="getScoreColor().bg"
              >
                <Icon :name="getScoreIcon()" class="h-8 w-8" :class="getScoreColor().text" />
              </div>
              <h2 class="text-foreground mb-2 text-3xl font-bold">Exam Completed!</h2>
              <div class="bg-card inline-flex items-center gap-4 rounded-lg border p-4">
                ...
                <div class="text-center">
                  <div class="text-2xl font-bold" :class="getScoreColor().text">
            ...
            <div>
              ...
              <div class="grid w-full max-w-6xl grid-cols-1 gap-6 pb-4 md:grid-cols-2">
                <div
                  ...
                  :key="question.id"
                  class="bg-card rounded-xl border p-4 transition-all hover:shadow-md"
                >
                  <!-- Question Header -->
                  <div class="mb-4 flex items-start gap-3">
                    ...
                    <Icon
                      :name="isAnswerCorrect(question) ? 'check-circle' : 'x-circle'"
                      class="h-5 w-5"
                      :class="isAnswerCorrect(question) ? 'text-green-500' : 'text-red-500'"
                    />
                  </div>
                  ...
                  <div class="mb-4 grid grid-cols-1 gap-4 p-4 md:grid-cols-2">
                    ...
                    <div class="space-y-2">
                      ...
                      <div class="bg-muted/50 rounded-lg p-3">
                        ...
                        <span v-else-if="question.type === 'true_false'" class="text-sm">
                          {{
                            userAnswers[question.id]?.[0] === 'true'
                              ? 'True'
                              : userAnswers[question.id]?.[0] === 'false'
                                ? 'False'
                                : 'Not answered'
                          }}
                        </span>
                        <span v-else class="text-sm">
                          {{
                            userAnswers[question.id]
                              ? question.options
                                  ?.filter((opt: Option) =>
                                    userAnswers[question.id].includes(opt.id)
                                  )
                    ...
                    <!-- Correct Answer -->
                    <div class="space-y-2">
                      <div class="text-muted-foreground text-sm font-medium">Correct Answer</div>
                      <div class="rounded-lg border border-green-200 bg-green-50 p-3">
                        ...
                        <span v-else class="text-sm text-green-700">
                          {{
                            question.options
                              ?.filter((opt: Option) => question.correctAnswers.includes(opt.id))
                              .map((opt: Option) => opt.text)
                              .join(', ')
                          }}
                        </span>
...
<script setup lang="ts">
  ...
  async function finishQuiz() {
    ...
    if (testMode.value) {
      const allQuestionsAnswered = quiz.value.questions.every(
        (question) => userAnswers.value[question.id] && userAnswers.value[question.id].length > 0
      );
  ...
  const isCurrentAnswerCorrect = computed(() => {
    ...
    if (currentQuestion.value.type === 'fill_in_blank') {
      ...
      return (
        userAnswer.length === currentQuestion.value.correctAnswers.length &&
  ...
  async function updateQuizSession(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        `/workspaces/${props.workspace_id}/quiz_sessions/${currentQuizSessionId.value}`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
